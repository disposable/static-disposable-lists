...
def validate_mailhosts(filename: str) -> bool:
    """
    Validate the given mail hosts file.

    Check that the JSON is valid and that the content matches the expected structure.
    The expected structure is a dictionary with service names as keys.
    ...
    for service_name, service_options in data.items():
        if not isinstance(service_options, dict):
            errors.append(f"Service {service_name} is not a dictionary")
            continue

        if "ip_addresses" in service_options:
            ip_addresses = service_options["ip_addresses"]

            if not isinstance(ip_addresses, list):
                errors.append(f"IP addresses for service {service_name} are not a list")
                continue

            for ip_address in ip_addresses:
                if not isinstance(ip_address, str):
                    errors.append(f"IP address {ip_address} in service {service_name} is not a string")
                    continue

                try:
                    ipaddress.ip_address(ip_address)
                except ValueError:
                    errors.append(f"Invalid IP address {ip_address} in service {service_name}")
                    continue

        if "mx_hosts" in service_options:
            mx_hosts_list = service_options["mx_hosts"]

            if not isinstance(mx_hosts_list, list):
                errors.append(f"MX hosts for service {service_name} are not a list")
                continue

            for mx_host in mx_hosts_list:
                if not isinstance(mx_host, str):
                    errors.append(f"MX host {mx_host} in service {service_name} is not a string")
                    continue

                if not HOSTNAME_RE.match(mx_host):
                    errors.append(f"Invalid MX host {mx_host} in service {service_name}")
                    continue

                if mx_host in mx_hosts:
                    errors.append(f"Duplicate MX host {mx_host} in service {service_name}")
                    continue
...
def main():
    ...
    parser.add_argument("--mail-hosts", help="Path to the mail-hosts JSON file.")
    parser.add_argument("--manual", help="Path to the manual disposable list file.")
    ...
    if args.github:
        ...
        args.mail_hosts = "disposable-mail-hosts.json"

        for filename in ('manual.txt', 'mail-data-hosts-net.txt'):
            if not validate_hostname_file(filename):
                error = True


    if args.manual and not validate_hostname_file(args.manual):
        error = True

    if args.mailservices and not validate_mailservices(args.mailservices):
        error = True

    if args.mail_hosts and not validate_mailhosts(args.mail_hosts):
        error = True
